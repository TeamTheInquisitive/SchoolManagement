...
## Seed Data via API (Production / Remote)

The `seed-data/` folder contains bash scripts that seed data directly via the REST API using `curl`. Use these to populate a **deployed** instance (e.g., Railway) without direct DB access.

### Quick Start

```bash
cd seed-data
./seed-all.sh                                              # Seeds everything (default: production URL)
./seed-all.sh http://localhost:8000/api/v1                 # Or target a local server
```

### Individual Scripts

| Script | What it creates | Records |
|--------|-----------------|---------|
| `seed-teachers.sh` | 30 teachers across all subjects | 30 |
| `seed-students.sh` | 200 students across 8-A/B, 9-A/B, 10-A/B | 200 |
| `seed-transport.sh` | Vehicles, drivers, helpers, routes | 70 |
| `seed-notifications.sh` | School announcements & reminders | 30 |
| `seed-examinations.sh` | 20 exams + ~580 student results | 600 |
| `seed-fees.sh` | Fee records for all classes (7 fee types) | ~1,250 |
| `seed-timetable.sh` | Period config + class-section slots | ~100 |

### Data Files

| File | Description |
|------|-------------|
| `students_200.json` | 200 student records (Indian names, distributed across classes 8-10, sections A-B) |

### Configuration

All scripts default to:
- **URL**: `https://schoolmanagement-prod.up.railway.app/api/v1`
- **Credentials**: `admin@school.com` / `password123`
- **School Code**: `SCH001`

Override with environment variables:
```bash
BASE_URL=http://localhost:8000/api/v1 ./seed-all.sh
```

### Prerequisites

- `curl` (available on all platforms)
- `python3` (for `seed-students.sh` and `seed-examinations.sh`)
- Valid admin credentials on the target server
- Classes (8, 9, 10), sections (A, B), and academic year (2025-2026) must already exist

### Notes

- Scripts are **idempotent for duplicates** — re-running returns 409 for existing records and continues
- Students require existing class-section-academic_year mappings in the DB
- Examination results are generated with realistic normal distributions (~65% mean)
- Fee records include a mix of paid (35%), partial (15%), overdue with late fees (8%), and pending (42%)
