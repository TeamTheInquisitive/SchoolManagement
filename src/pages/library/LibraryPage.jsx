...
import { Plus, Download, ArrowLeftRight, BookOpen, AlertTriangle, DollarSign } from 'lucide-react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import api from '../../services/api';
import { ENDPOINTS } from '../../config/api';
import { Button, SearchInput, Badge, Modal, SearchableSelect, Tabs, DateInput, useToast, exportToCsv, Pagination, usePagination, useTabState } from 'school-erp-ui-shared';
...
export default function LibraryPage() {
  ...
  const issued = Array.isArray(issuedData?.results) ? issuedData.results : [];
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 md:p-5 transition-all duration-200 hover:shadow-soft-lg hover:border-slate-300">
        <div className="flex justify-between mb-3"><SearchInput value={search} onChange={(v) => { setSearch(v); pagination.reset(); }} placeholder={tab === 0 ? "Search books..." : "Search issues..."} /><Button variant="secondary" size="sm" icon={Download} onClick={() => { if (tab === 0) { const headers = ['Title', 'Author', 'ISBN', 'Category', 'Total Copies', 'Available', 'Issued']; const rows = books.map(b => [b.title, b.author, b.isbn, b.category, b.total_copies, b.available_copies, (b.total_copies || 0) - (b.available_copies || 0)]); exportToCsv('library-books', headers, rows); } else { const headers = ['Book', 'Student', 'Issue Date', 'Due Date', 'Penalty', 'Status']; const rows = issued.map(i => [i.book_title || i.title, i.student_name || i.user_name, i.issue_date, i.due_date, i.fine_amount || 0, i.status]); exportToCsv('library-issues', headers, rows); } toast.success('CSV exported successfully'); }}>Export CSV</Button></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {books.length === 0 && <tr><td colSpan={7} className="text-center py-8 text-slate-400">No books found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {issued.length === 0 && <tr><td colSpan={7} className="text-center py-8 text-slate-400">No issue records found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
