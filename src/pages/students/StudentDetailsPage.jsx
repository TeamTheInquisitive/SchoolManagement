...
export default function StudentDetailsPage() {
  ...
  const [meetingForm, setMeetingForm] = useState({ meeting_date: '', meeting_type: '', agenda: '', discussion_notes: '', remarks: '', follow_up_required: false, next_meeting_date: '' });
  ...
  const handleOpenMeetingModal = (meeting = null) => {
    if (meeting) {
      ...
      setMeetingForm({ meeting_date: meeting.date || '', meeting_type: meeting.type || '', agenda: meeting.agenda || '', discussion_notes: meeting.notes || '', remarks: meeting.remarks || '', follow_up_required: meeting.follow_up_required || false, next_meeting_date: meeting.next_meeting_date || '', status: meeting.status || '' });
    } else {
      setEditingMeeting(null);
      setMeetingForm({ meeting_date: '', meeting_type: '', agenda: '', discussion_notes: '', remarks: '', follow_up_required: false, next_meeting_date: '' });
    }
  ...
  const handleEdit = () => {
    setEditForm({ full_name: raw.full_name || '', email: raw.email || '', phone: raw.phone || '', class_name: raw.class_name || '', section: raw.section || '', gender: raw.gender || '', date_of_birth: raw.date_of_birth || '', address: raw.address || '' });
    setEditOpen(true);
  };
  ...
  const handleEditSection = (section) => {
    const formData = {
      ...
      parent: { parent_name: raw.parent?.name || '', parent_phone: raw.parent?.phone || '', parent_email: raw.parent?.email || '', parent_emergency: raw.parent?.emergency_contact || '', parent_relationship: raw.parent?.relationship || '' },
  ...
  const handleSaveSection = () => {
    const allowedFields = ['full_name', 'email', 'phone', 'date_of_birth', 'admission_date', 'gender', 'student_type', 'blood_group', 'religion', 'address', 'address_line2', 'city', 'state', 'pincode', 'status', 'class_name', 'section', 'parent_name', 'parent_phone', 'parent_email', 'parent_relationship'];
