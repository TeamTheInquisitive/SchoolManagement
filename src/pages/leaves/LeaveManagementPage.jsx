...
import { Button, Badge, Modal, SearchableSelect, useToast, useSortableData, Breadcrumb, HoverCard, usePagination, DataTable , AnimatedNumber } from 'school-erp-ui-shared';
...
export default function LeaveManagementPage() {
  ...
  const columns = [
    ...
    { key: 'reason', label: 'Reason', render: (l) => <span className="max-w-[160px] truncate block">{l.reason}</span> },
    { key: 'status', label: 'Status', sortable: true, render: (l) => <Badge status={l.status} /> },
    {
      key: 'actions', label: 'Actions',
      render: (l) => l.status === 'Pending' ? (
        <div className="flex gap-1">
          <button onClick={() => setViewLeave(l)} className="p-1 hover:bg-slate-100 rounded"><Eye className="w-4 h-4 text-slate-500" /></button>
          <button onClick={() => approveMutation.mutate({ id: l.id, data: {} }, { onSuccess: () => toast.success('Leave approved'), onError: (err) => toast.error(err.response?.data?.error || err.response?.data?.detail || 'Failed to approve') })} className="p-1 hover:bg-green-50 rounded"><CheckCircle className="w-4 h-4 text-green-600" /></button>
          <button onClick={() => { setRejectData({ id: l.id, teacher_name: l.teacher_name }); setRejectRemarks(''); }} className="p-1 hover:bg-red-50 rounded"><XCircle className="w-4 h-4 text-red-500" /></button>
        </div>
      ) : <button onClick={() => setViewLeave(l)} className="p-1 hover:bg-slate-100 rounded"><Eye className="w-4 h-4 text-slate-500" /></button>,
    },
  ];
    ...
    <div>
      ...
      <Modal open={allocateOpen} onClose={() => setAllocateOpen(false)} title="Allocate Leaves to Employees" size="lg">
        ...
        <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-slate-100">
          ...
          <Button variant="primary" disabled={allocateForm.staff_ids.length === 0 || allocateMutation.isPending} onClick={() => {
            const leave_types = {};
            ...
            allocateMutation.mutate({ staff_ids: allocateForm.staff_ids, leave_types }, { onSuccess: () => { setAllocateOpen(false); setAllocateForm({ staff_ids: [], leave_types: {} }); toast.success('Leaves allocated successfully'); }, onError: (err) => { toast.error(err.response?.data?.error || err.response?.data?.detail || 'Failed to allocate leaves'); } });
          }}>{allocateMutation.isPending ? 'Allocating...' : `Allocate to ${allocateForm.staff_ids.length} Employee(s)`}</Button>
        </div>
      </Modal>
        ...
        {viewLeave && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              {viewLeave.approved_by && <div><p className="text-xs text-slate-500">Reviewed By</p><p className="font-medium">{viewLeave.approved_by}</p></div>}
            </div>
            {viewLeave.status === 'Pending' && (
              <div className="flex gap-2 pt-3 border-t border-slate-100">
                <Button variant="primary" onClick={() => { approveMutation.mutate({ id: viewLeave.id, data: {} }, { onSuccess: () => toast.success('Leave approved'), onError: (err) => toast.error(err.response?.data?.error || err.response?.data?.detail || 'Failed to approve') }); setViewLeave(null); }}>Approve</Button>
                <Button variant="ghost" onClick={() => { setRejectData({ id: viewLeave.id, teacher_name: viewLeave.teacher_name }); setRejectRemarks(''); setViewLeave(null); }}>Reject</Button>
              </div>
            )}
          </div>
        )}
      ...
      <Modal open={!!rejectData} onClose={() => setRejectData(null)} title="Reject Leave Request">
        {rejectData && (
