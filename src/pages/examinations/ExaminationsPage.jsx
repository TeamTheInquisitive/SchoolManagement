...
import { useClassSubjects, useSchoolProfile } from '../../services/settingsService';
import { getSchoolInfo } from '../../utils/getSchoolInfo';
import api from '../../services/api';
import { ENDPOINTS } from '../../config/api';
import { Button, SearchInput, Badge, Modal, SearchableSelect, ConfirmDialog, DateInput, TimeInput, useToast, exportStyledExcel, exportStyledMultiSheetExcel, Breadcrumb, EmptyState, Pagination, usePagination, ErrorBoundary , AnimatedNumber } from 'school-erp-ui-shared';
...
export default function ExaminationsPage() {
  ...
  const handleDownloadReport = async (exam) => {
    try {
      const res = await api.get(ENDPOINTS.examinations.results(exam.id));
      const results = res.data?.results || [];
      if (!results.length) { toast.info('No results available for this exam'); return; }
      const headers = ['Student Name', 'Roll Number', 'Marks Obtained', 'Total Marks', 'Percentage', 'Grade', 'Rank', 'Attendance'];
      const rows = results.map(r => [
        r.student_name || r.full_name || '',
        r.roll_number || r.admission_number || '',
        r.marks_obtained ?? r.marks ?? '',
        exam.total_marks || r.total_marks || '',
        r.marks_obtained != null && exam.total_marks ? `${((r.marks_obtained / exam.total_marks) * 100).toFixed(1)}%` : '',
        r.grade || '',
        r.rank || '',
        r.attendance || '',
      ]);
      const filename = `${exam.subject || exam.exam_name || 'Exam'}_${exam.class_section || ''}_${exam.date || ''}_Results`;
      await exportStyledExcel({ filename, schoolInfo: getSchoolInfo(schoolProfile), reportTitle: `${exam.name || exam.subject || 'Exam'} Results`, headers, rows });
  ...
  const handleDownloadMultiSubjectReport = async (typeExams, typeName, className, sectionKey) => {
    try {
      ...
      for (const exam of typeExams) {
        const res = await api.get(ENDPOINTS.examinations.results(exam.id));
        const results = res.data?.results || [];
        const subjectName = exam.subject || exam.subject_name || `Subject ${sheets.length + 1}`;
        const sheetName = subjectName.slice(0, 31);
        const headers = ['Roll Number', 'Student Name', 'Marks Obtained', 'Total Marks', 'Percentage', 'Grade', 'Rank'];
        const rows = results.map(r => [
          r.roll_number || r.admission_number || '',
          r.student_name || r.full_name || '',
          r.marks_obtained ?? r.marks ?? '',
      ...
      if (!sheets.length || sheets.every(s => s.rows.length === 0)) {
        toast.info('No results available for this exam');
        return;
      }
      const filename = `${typeName}_${className}_Section_${sectionKey}_Report`;
      await exportStyledMultiSheetExcel({ filename, schoolInfo: getSchoolInfo(schoolProfile), reportTitle: `${typeName} - ${className} Section ${sectionKey}`, sheets });
      toast.success(`Report downloaded with ${sheets.length} subject tabs`);
    } catch (err) {
  ...
  const handleSubmit = () => {
    ...
    const payload = { ...form, total_marks: Number(form.total_marks), exam_type: form.type, passing_marks: form.passing_marks ? Number(form.passing_marks) : null };
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Schedule exams and manage results</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" icon={Download} onClick={() => setReportDialogOpen(true)}>Download Report</Button>
                                  ...
                                  <div key={typeName} className="bg-white border border-slate-100 rounded-lg overflow-hidden">
                                    <div className="flex items-center justify-between px-4 py-2 bg-gradient-to-r from-primary-50/50 to-white border-b border-slate-100">
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        {typeAllPast && <button onClick={() => handleDownloadMultiSubjectReport(typeExams, typeName, group.displayName, sectionKey)} className="p-1 hover:bg-emerald-50 rounded" title="Download Report (All Subjects)"><Download size={12} className="text-emerald-500" /></button>}
                                            ...
                                            <tr key={e.id} className="hover:bg-primary-50/30 transition-colors duration-150">
                                              ...
                                              <td className="py-2 px-4">
                                                <div className="flex gap-1">
                                                  ...
                                                  {isPast && <button onClick={() => handleDownloadReport(e)} className="p-1 hover:bg-emerald-50 rounded" title="Download Report"><Download size={13} className="text-emerald-500" /></button>}
                                                </div>
                                              </td>
                                            </tr>
                                          );
      ...
      <Modal open={examDialog.open} onClose={() => setExamDialog({ open: false, data: null })} title={examDialog.data?.id ? 'Edit Exam' : 'Schedule New Exam'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 mb-1.5 block">Passing Marks</label>
              <input type="number" value={form.passing_marks || ''} onChange={e => setForm({ ...form, passing_marks: e.target.value })} placeholder="Optional" className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-150 hover:border-slate-400" />
              <p className="text-[11px] text-slate-400 mt-1">If left empty, pass/fail is determined by the grade system (Settings → Grades)</p>
            </div>
            <div>
...
function ExamResultsView({ exam }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {results.length > 0 && (() => {
        const classAvg = stats.class_average || (results.reduce((s, r) => s + (r.percentage || ((r.marks || 0) / maxMarks * 100)), 0) / results.length);
        const highest = stats.highest_score || Math.max(...results.map(r => r.marks || 0));
        const lowest = stats.lowest_score || Math.min(...results.map(r => r.marks || 0));
        const passRate = results.length > 0 ? ((passCount / results.length) * 100).toFixed(1) : 0;
        return (
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center">
              <p className="text-xl font-bold text-blue-700">{classAvg.toFixed(1)}%</p>
              <p className="text-[11px] text-blue-500 mt-0.5">Class Average</p>
            </div>
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-center">
              <p className={`text-xl font-bold ${Number(passRate) >= 70 ? 'text-emerald-700' : Number(passRate) >= 50 ? 'text-amber-700' : 'text-red-700'}`}>{passRate}%</p>
              <p className="text-[11px] text-indigo-500 mt-0.5">Pass Rate</p>
            </div>
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-center">
              <p className="text-xl font-bold text-emerald-700">{highest}<span className="text-xs text-emerald-400">/{maxMarks}</span></p>
              <p className="text-[11px] text-emerald-500 mt-0.5">Highest Marks</p>
            </div>
            <div className="bg-red-50 border border-red-100 rounded-xl p-3 text-center">
              <p className="text-xl font-bold text-red-700">{lowest}<span className="text-xs text-red-400">/{maxMarks}</span></p>
              <p className="text-[11px] text-red-500 mt-0.5">Lowest Marks</p>
            </div>
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-center">
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-lg p-4 max-h-[300px] overflow-y-auto space-y-3">
              ...
              {allClasses.map((cls, idx) => {
                ...
                const allSectionsSelected = hasSections ? cls.sections.every(sec => allClassesSelected || selectedSections[`${cls.id}-${sec.id}`]) : !!selectedSections[`${cls.id}-all`];
                const someSectionsSelected = hasSections ? cls.sections.some(sec => allClassesSelected || selectedSections[`${cls.id}-${sec.id}`]) : !!selectedSections[`${cls.id}-all`];
                return (
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={() => {
                            setSelectedSections(prev => {
                              const updated = { ...prev };
                              const isCurrentlySelected = hasSections
                                ? cls.sections.every(sec => !!updated[`${cls.id}-${sec.id}`])
                                : !!updated[`${cls.id}-all`];
                              if (isCurrentlySelected) {
                                if (hasSections) {
                                  cls.sections.forEach(sec => { delete updated[`${cls.id}-${sec.id}`]; });
                                } else {
                                  delete updated[`${cls.id}-all`];
                                }
                              } else {
                                if (hasSections) {
                                  cls.sections.forEach(sec => { updated[`${cls.id}-${sec.id}`] = true; });
                                } else {
                                  updated[`${cls.id}-all`] = true;
                                }
                              }
                              return updated;
                            });
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {otherKeys.length > 0 && (
                      <div className="flex items-center gap-2 min-w-[200px]">
                        <span className="text-xs text-slate-500 flex-shrink-0">Copy from:</span>
                        <SearchableSelect
                          value=""
                          onChange={(v) => { if (v) copyFrom(pair.key, v); }}
                          options={[{ value: '', label: 'Select section...' }, ...otherKeys]}
...
// --- Download Report Modal ---
function DownloadReportModal({ open, onClose, allClasses, subjects, toast, schoolProfile }) {
  ...
  // Fetch ALL exams across pages (backend caps page_size at 100) so type/title
  // options and the downloaded report are complete, not limited to one page.
  const { data: allExamsData, isLoading: examsLoading } = useQuery({
    queryKey: ['exam-report-options'],
    queryFn: async () => {
      const pageSize = 100;
      let page = 1;
      let total = Infinity;
      ...
      while (all.length < total) {
        const r = await api.get(ENDPOINTS.examinations.list, { params: { page, page_size: pageSize } });
  ...
  const allExams = useMemo(() => Array.isArray(allExamsData?.results) ? allExamsData.results : [], [allExamsData]);
  ...
  // Exam type options derived from existing exams (falls back to EXAM_TYPES)
  const typeOptions = useMemo(() => {
    const fromData = [...new Set(allExams.map(e => e.type || e.exam_type).filter(Boolean))];
    const values = fromData.length > 0 ? fromData : EXAM_TYPES.map(t => t.value);
    return values.map(v => ({ value: v, label: v }));
  }, [allExams]);

  // Exam titles populated based on the selected type
  const titleOptions = useMemo(() => {
    if (!selectedType) return [];
    const names = [...new Set(
      allExams
        .filter(e => (e.type || e.exam_type) === selectedType)
        .map(e => e.name)
        .filter(Boolean)
    )];
  ...
  // Exams matching the chosen type + title (used to scope class/subject options)
  const matchingExams = useMemo(() => {
    if (!selectedType || !selectedTitle) return [];
    return allExams.filter(e => (e.type || e.exam_type) === selectedType && e.name === selectedTitle);
  }, [allExams, selectedType, selectedTitle]);

  // Class options scoped to the selected exam title (falls back to all classes)
  const classOptions = useMemo(() => {
    const fromExams = [...new Set(matchingExams.map(e => e.class_name).filter(Boolean))];
    if (fromExams.length > 0) {
      return fromExams.map(name => {
        const cls = allClasses.find(c => c.name === name);
        return { value: name, label: cls?.display_name || name };
      });
    }
    return allClasses.map(cls => ({ value: cls.name, label: cls.display_name || cls.name }));
  }, [matchingExams, allClasses]);

  // Subject options scoped to the selected title + chosen classes
  const subjectOptions = useMemo(() => {
    const scoped = matchingExams.filter(e => selectedClasses.length === 0 || selectedClasses.includes(e.class_name));
    const fromExams = [...new Set(scoped.map(e => e.subject).filter(Boolean))];
    if (fromExams.length > 0) return fromExams.map(s => ({ value: s, label: s }));
    return subjects.map(s => ({ value: s.name, label: s.name }));
  }, [matchingExams, selectedClasses, subjects]);
  ...
  const toggleSubject = (subjectName) => {
    setSelectedSubjects(prev => prev.includes(subjectName) ? prev.filter(s => s !== subjectName) : [...prev, subjectName]);
  };

  const handleDownload = async () => {
    if (!selectedType) { toast.error('Please select an exam type'); return; }
    if (!selectedTitle) { toast.error('Please select an exam title'); return; }
    ...
    // Scope exams to the selected classes and subjects
    const examsToExport = matchingExams.filter(e =>
      selectedClasses.includes(e.class_name) && selectedSubjects.includes(e.subject)
    );
    if (examsToExport.length === 0) { toast.error('No exams found for the selected classes/subjects'); return; }
    ...
    try {
      ...
      for (const exam of examsToExport) {
        const res = await api.get(ENDPOINTS.examinations.results(exam.id));
        const results = res.data?.results || [];
        const headers = ['Roll Number', 'Student Name', 'Class', 'Section', 'Subject', 'Marks Obtained', 'Total Marks', 'Percentage', 'Grade', 'Rank'];
        const rows = results.map(r => [
          r.roll_number || r.admission_number || '',
          r.student_name || r.full_name || '',
          exam.class_name || '',
          ...
          r.marks_obtained ?? r.marks ?? '',
          exam.total_marks || r.total_marks || '',
          r.marks_obtained != null && exam.total_marks ? `${((r.marks_obtained / exam.total_marks) * 100).toFixed(1)}%` : (r.percentage != null ? `${Number(r.percentage).toFixed(1)}%` : ''),
          r.grade || '',
          r.rank || '',
        ]);
        // Sheet name: Class-Section Subject (Excel sheet names max 31 chars)
        const sheetName = `${exam.class_name || ''}-${exam.section || ''} ${exam.subject || ''}`.trim().slice(0, 31);
        sheets.push({ name: sheetName || `Sheet ${sheets.length + 1}`, headers, rows });
      }

      if (sheets.length === 0 || sheets.every(s => s.rows.length === 0)) {
        toast.info('No results available for the selected exam');
        setGenerating(false);
        return;
      }

      const filename = `${selectedTitle}_Results_${selectedClasses.join('_')}`;
      await exportStyledMultiSheetExcel({ filename, schoolInfo: getSchoolInfo(schoolProfile), reportTitle: `${selectedTitle} - Results`, sheets });
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Download Exam Report" size="lg">
      <div className="space-y-5">
        <p className="text-sm text-slate-500">Select an exam type and title, then choose the classes and subjects to download their results as an Excel report. Each class-section-subject is exported as a separate tab.</p>
        ...
        <div>
          ...
          <SearchableSelect
            ...
            options={[{ value: '', label: 'Select exam type...' }, ...typeOptions]}
            placeholder="Select exam type..."
            showSearch
          />
        </div>

        {/* Exam Title Selection (populated based on type) */}
        <div>
          <label className="text-sm font-medium text-slate-700 mb-1.5 block">Exam Title *</label>
          <SearchableSelect
            value={selectedTitle}
            onChange={handleSelectTitle}
            options={[{ value: '', label: 'Select exam title...' }, ...titleOptions]}
            placeholder={selectedType ? 'Select exam title...' : 'Select a type first'}
            showSearch
          />
          {selectedType && !examsLoading && titleOptions.length === 0 && (
            <p className="text-xs text-slate-400 mt-1">No exams found for this type.</p>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-sm font-medium text-slate-700">Class * <span className="text-xs text-slate-400 font-normal">(multi-select)</span></label>
            {classOptions.length > 0 && (
              <label className="flex items-center gap-1.5 cursor-pointer text-xs font-medium text-primary-600 hover:text-primary-700">
                <input type="checkbox" checked={selectedClasses.length === classOptions.length && classOptions.length > 0} onChange={() => { if (selectedClasses.length === classOptions.length) setSelectedClasses([]); else setSelectedClasses(classOptions.map(c => c.value)); }} className="w-3.5 h-3.5 rounded border-slate-300 text-primary-600 focus:ring-primary-500" />
                Select All
              </label>
            )}
          </div>
          <div className="border border-slate-200 rounded-lg p-3 max-h-[180px] overflow-y-auto">
            {classOptions.length === 0 ? (
              <p className="text-xs text-slate-400">Select an exam title first.</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {classOptions.map(cls => {
                  ...
                  return (
                    <label
                      key={cls.value}
                      className={`flex items-center gap-1.5 cursor-pointer px-3 py-1.5 rounded-lg border text-sm transition-all duration-150 ${isSelected ? 'bg-primary-50 border-primary-300 text-primary-700 font-medium' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}
                    >
                      <input type="checkbox" checked={isSelected} onChange={() => toggleClass(cls.value)} className="w-3.5 h-3.5 rounded border-slate-300 text-primary-600 focus:ring-primary-500" />
                      {cls.label}
                    </label>
                  );
                })}
          ...
          {selectedClasses.length > 0 && <p className="text-xs text-slate-500 mt-1">{selectedClasses.length} class(es) selected</p>}
        </div>

        {/* Subject Multi-Select */}
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-sm font-medium text-slate-700">Subjects * <span className="text-xs text-slate-400 font-normal">(multi-select — each becomes a tab)</span></label>
            {subjectOptions.length > 0 && (
              <label className="flex items-center gap-1.5 cursor-pointer text-xs font-medium text-emerald-600 hover:text-emerald-700">
                <input type="checkbox" checked={selectedSubjects.length === subjectOptions.length && subjectOptions.length > 0} onChange={() => { if (selectedSubjects.length === subjectOptions.length) setSelectedSubjects([]); else setSelectedSubjects(subjectOptions.map(s => s.value)); }} className="w-3.5 h-3.5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" />
                Select All
              </label>
            )}
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {subjectOptions.map(sub => {
                  ...
                  return (
                    <label
                      key={sub.value}
                      className={`flex items-center gap-1.5 cursor-pointer px-3 py-1.5 rounded-lg border text-sm transition-all duration-150 ${isSelected ? 'bg-emerald-50 border-emerald-300 text-emerald-700 font-medium' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}
                    >
                      <input type="checkbox" checked={isSelected} onChange={() => toggleSubject(sub.value)} className="w-3.5 h-3.5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" />
                      {sub.label}
                    </label>
                  );
                })}
          ...
          {selectedSubjects.length > 0 && <p className="text-xs text-slate-500 mt-1">{selectedSubjects.length} subject(s) selected — will generate {selectedSubjects.length} tab(s)</p>}
        </div>
      </div>

      <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-slate-100">
        <Button variant="ghost" onClick={handleClose}>Cancel</Button>
        <Button icon={Download} onClick={handleDownload} loading={generating} disabled={!selectedType || !selectedTitle || selectedClasses.length === 0 || selectedSubjects.length === 0}>
          Download Report
        </Button>
      </div>
    </Modal>
