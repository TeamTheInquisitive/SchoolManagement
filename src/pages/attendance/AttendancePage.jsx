...
import { Skeleton, useToast, Breadcrumb, DatePicker, AttendanceMarkingView } from 'school-erp-ui-shared';
import { CheckCircle2, Clock, CalendarDays, Search, Users, ArrowLeft, BookOpen, User } from 'lucide-react';
import api from '../../services/api';
import { ENDPOINTS } from '../../config/api';
import { useClassSections, useAcademicYear, useAttendanceConfig, useHolidays } from '../../services/settingsService';
...
export default function AttendancePage() {
  ...
  const attendanceMode = attendanceConfig?.attendance_mode || 'daily';
  const isSubjectWise = attendanceMode === 'subject_wise';

  const holidays = holidaysData?.holidays || holidaysData?.results || (Array.isArray(holidaysData) ? holidaysData : []);
  const isWeekend = (() => { const d = new Date(date + 'T00:00:00'); const day = d.getDay(); return day === 0; })();
  ...
  const holidayWarning = isHolidayOrWeekend
    ? (matchedHoliday ? `Holiday: ${matchedHoliday.name || matchedHoliday.title || 'School Holiday'}` : 'This is a Sunday')
    : null;
