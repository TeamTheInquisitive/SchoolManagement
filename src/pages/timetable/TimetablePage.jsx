...
import { usePeriods, useTimetable, useAssignSlot, useDeleteSlot, useResetSlots, useSubjects, useTeachers, useTeacherAvailability, useCreatePeriod, useUpdatePeriod, useDeletePeriod, useSlotTypes } from '../../services/timetableService';
...
import { Button, Modal, ConfirmDialog, SearchableSelect, Checkbox, FilterBar, TimeInput, useToast, Breadcrumb, TimetableGrid } from 'school-erp-ui-shared';
import { useSchoolProfile } from '../../services/settingsService';
import { getSchoolInfo } from '../../utils/getSchoolInfo';
import { generateTimetablePdf } from 'school-erp-ui-shared';
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          {classFilter && sectionFilter && selectedCS && (
            <Button variant="secondary" size="sm" icon={Trash2} className="!text-red-600 !border-red-200 hover:!bg-red-50" onClick={() => setResetConfirmOpen(true)}>
              Reset All Slots
            </Button>
          )}
        </div>
      ...
      {/* Download PDF */}
      {classFilter && sectionFilter && periods.length > 0 && (
        <div className="flex justify-end mt-4">
          <Button variant="primary" size="lg" icon={Download} onClick={() => { generateTimetablePdf({ schoolInfo: getSchoolInfo(schoolProfile), className: classFilter, section: sectionFilter, periods, days: DAYS, slots, academicYear: timetableData?.academic_year }); toast.success('Timetable PDF downloaded'); }}>Download PDF</Button>
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          resetSlots.mutate(selectedCS?.id, {
            onSuccess: (data) => { setResetConfirmOpen(false); toast.success(data.message || 'All slots reset successfully'); },
            onError: (err) => { setResetConfirmOpen(false); toast.error(err.response?.data?.error || 'Failed to reset slots'); },
          });
        }}
        loading={resetSlots.isPending}
        title="Reset All Slots"
        message={`This will permanently delete all timetable slots for Class ${classFilter} Section ${sectionFilter}. This action cannot be undone.`}
...
function SlotFormDialog({ open, onClose, slot, subjects, teachers, classSection, periods, onSave, sectionAssignments }) {
  ...
  const rawBusyTeachers = availabilityData?.busy_teachers || {};
  // Extract current class-section identifier (e.g., "5-A") to exclude from busy list
  // classSection prop is "Class 5-A", busy info uses "5-A"
  const currentCS = classSection ? classSection.replace(/^Class\s+/i, '') : '';
  const busyTeachers = Object.fromEntries(
    Object.entries(rawBusyTeachers).filter(([_, info]) => info.class !== currentCS)
  );
  ...
  return (
    <Modal open={open} onClose={onClose} title={slot.existing ? 'Edit Timetable Slot' : 'Add Timetable Slot'} size="xl">
      {/* Slot Info Header */}
