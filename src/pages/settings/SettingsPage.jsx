...
import { useSchoolProfile, useAcademicYear, useAcademicYears, useCreateAcademicYear, useUpdateAcademicYearById, useDeleteAcademicYear, useSetCurrentAcademicYear, useClonePreview, useInitializeFromYear, useClassSections, useSubjects, useUpdateSchoolProfile, useUpdateAcademicYear, useCreateClasses, useCreateSections, useDeleteClass, useDeleteClassSection, useCreateSubjects, useUpdateSubject, useDeleteSubject, useClassSubjects, useUpdateClassSubjects, useFeeStructures, useCreateFeeStructure, useUpdateFeeStructure, useDeleteFeeStructure, useUploadLogo, useIdGenerationConfig, useUpdateIdGenerationConfig, useCheckPrefix, useHolidays, useUpdateHolidays, useAttendanceConfig, useUpdateAttendanceConfig, useClassSectionAssignments, useUpdateClassSectionAssignment } from '../../services/settingsService';
...
function IdGenerationSection() {
  ...
  const entities = [
    { key: 'student', label: 'Student', suffix: '' },
    { key: 'teacher', label: 'Teacher', suffix: 'T' },
    { key: 'staff', label: 'Staff', suffix: 'S' },
  ];

  const { data: schoolData } = useSchoolProfile();

  // Initialize prefix from config, or auto-derive from school name
  useEffect(() => {
    if (config?.prefix) {
      setPrefix(config.prefix);
    } else if (schoolData?.name && !prefix) {
      const derived = schoolData.name.replace(/\s+/g, '').toUpperCase().slice(0, 3);
      if (derived.length >= 2) setPrefix(derived);
    }
  }, [config, schoolData]);
  ...
  // Availability check
  const { data: prefixCheck, isFetching: checking } = useCheckPrefix(debouncedPrefix);

  if (isLoading) return null;

  const currentConfig = config || {};

  const getEnabledState = (key) => {
    if (toggleState && toggleState[key] !== undefined) return toggleState[key];
    return currentConfig[key]?.enabled || false;
  };

  const handleToggle = (key) => {
    setToggleState(prev => ({ ...(prev || {}), [key]: !getEnabledState(key) }));
  };

  const getPattern = (entityKey) => {
    if (!prefix) return '';
    const entity = entities.find(e => e.key === entityKey);
    return `${prefix}${entity.suffix}{YY}{SEQ:4}`;
  };

  const getPreview = (entityKey) => {
    const pattern = getPattern(entityKey);
    if (!pattern) return '—';
    const yy = new Date().getFullYear().toString().slice(-2);
    return pattern.replace('{YY}', yy).replace('{SEQ:4}', '0001');
  };
  ...
  const handleSave = () => {
    ...
    entities.forEach(({ key }) => {
      payload[key] = {
        enabled: getEnabledState(key),
        pattern: getPattern(key),
        next_seq: currentConfig[key]?.next_seq || 1,
      };
    });
    updateMutation.mutate(payload, {
      onSuccess: () => { setToggleState(null); toast.success('ID generation settings saved'); },
      onError: (err) => toast.error(err.response?.data?.detail || 'Failed to save'),
    });
  };

  const isDirty = (prefix && prefix !== (config?.prefix || '')) || toggleState !== null;

  const prefixValid = prefix.length >= 2;
  const prefixAvailable = prefixCheck?.available;
  const prefixIsCurrentSchool = debouncedPrefix === (config?.prefix || '');
    ...
    <div className="mt-6 bg-white border border-slate-200 rounded-xl p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Configure your school ID prefix to auto-generate IDs for students, teachers, and staff</p>
        </div>
        {isDirty && <Button size="sm" icon={Save} onClick={handleSave} loading={updateMutation.isPending} disabled={!prefixValid || (checking && !prefixIsCurrentSchool) || (!prefixIsCurrentSchool && prefixCheck && !prefixAvailable)}>Save</Button>}
      </div>

      {/* Prefix Input */}
      <div className="mb-4 p-4 bg-slate-50 rounded-lg border border-slate-100">
        <label className="text-sm font-medium text-slate-700 mb-1.5 block">School ID Prefix</label>
        <div className="flex items-center gap-3">
          <input
            ...
            placeholder="e.g. SPR"
            maxLength={6}
            className="w-40 text-sm font-mono font-bold uppercase border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent tracking-wider"
          />
          <div className="text-xs">
            {!prefix && <span className="text-slate-400">Enter 2-6 characters</span>}
            {prefix && prefix.length < 2 && <span className="text-amber-600">Min 2 characters required</span>}
            {prefixValid && prefixIsCurrentSchool && <span className="text-emerald-600 font-medium flex items-center gap-1"><span>&#10003;</span> Current prefix</span>}
            {prefixValid && !prefixIsCurrentSchool && checking && <span className="text-slate-500 flex items-center gap-1"><span className="inline-block w-3 h-3 border-2 border-slate-300 border-t-primary-600 rounded-full animate-spin"></span> Checking...</span>}
            {prefixValid && !prefixIsCurrentSchool && !checking && prefixAvailable && <span className="text-emerald-600 font-medium flex items-center gap-1"><span>&#10003;</span> Available</span>}
            {prefixValid && !prefixIsCurrentSchool && !checking && prefixCheck && !prefixAvailable && <span className="text-red-600 font-medium flex items-center gap-1"><span>&#10007;</span> Already taken</span>}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {entities.map(({ key, label, suffix }) => {
          const enabled = getEnabledState(key);
          return (
            <div key={key} className="flex items-center gap-4 p-3 bg-slate-50 rounded-lg border border-slate-100">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={enabled} onChange={() => handleToggle(key)} className="sr-only peer" />
                <div className="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary-600"></div>
              </label>
              <input
                type="text"
                value={getPattern(key)}
                readOnly
                className="flex-1 text-sm border border-slate-200 rounded-lg px-3 py-1.5 bg-slate-100 text-slate-600 cursor-not-allowed"
              />
              <div className="text-xs text-slate-500 min-w-[120px]">
                Preview: <span className="font-mono text-slate-700">{getPreview(key)}</span>
              </div>
            </div>
          );
        })}
      </div>
      <p className="text-[11px] text-slate-400 mt-3">Pattern format: <code className="bg-slate-100 px-1 rounded">{'{PREFIX}'}</code><code className="bg-slate-100 px-1 rounded">{'{YY}'}</code><code className="bg-slate-100 px-1 rounded">{'{SEQ:4}'}</code>. Teachers get a "T" suffix, staff get an "S" suffix after the prefix. Example: SPR260001 (student), SPRT260001 (teacher), SPRS260001 (staff)</p>
    </div>
  );
}
...
function AcademicYearTab() {
  ...
  return (
    <div className="space-y-6">
              ...
              <div key={year.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                {editingId === year.id ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2">
                      {!year.is_current && (
                        <button onClick={() => setCloneTarget(year)} className="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Initialize</button>
                      )}
                      {!year.is_current && (
      ...
      {cloneTarget && <CloneYearModal open={!!cloneTarget} onClose={() => setCloneTarget(null)} targetYear={cloneTarget} allYears={years} />}
    </div>
  );
}
...
function CloneYearModal({ open, onClose, targetYear, allYears }) {
  ...
  const { data: preview, isLoading: previewLoading } = useClonePreview(sourceYearId || null);
  ...
  const totalSelected = preview
    ? Object.entries(modules).filter(([_, v]) => v).reduce((sum, [key]) => sum + (preview.modules?.[key]?.count || 0), 0)
    : 0;
  ...
  const handleClone = () => {
    cloneMutation.mutate(
      ...
      {
        onSuccess: (data) => { toast.success(`Initialized ${data.total_records_cloned} records from ${data.source_year}`); onClose(); },
        onError: (err) => { toast.error(err.response?.data?.detail || err.response?.data?.error || 'Clone failed'); },
      }
    );
  };

  return (
    <Modal open={open} onClose={onClose} title={`Initialize ${targetYear.name} from Previous Year`} size="lg">
      <div className="space-y-5">
        {/* Source Year Selector */}
        <div>
          <label className="block text-xs font-medium text-slate-600 mb-1.5">Copy from</label>
          <select value={sourceYearId} onChange={(e) => setSourceYearId(e.target.value)} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            <option value="">Select source year...</option>
            {allYears.filter(y => y.id !== targetYear.id).map(y => (
              ...
              <table className="w-full text-sm border border-slate-200 rounded-lg overflow-hidden">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="text-left px-3 py-2 font-semibold text-slate-700 text-xs">Module</th>
                    <th className="text-left px-3 py-2 font-semibold text-slate-700 text-xs">What will be copied</th>
                    <th className="text-right px-3 py-2 font-semibold text-slate-700 text-xs">Records</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.entries(preview.modules).map(([key, mod]) => (
                    <tr key={key} className="border-t border-slate-100 hover:bg-slate-50/50">
                      <td className="px-3 py-2.5">
                        <input type="checkbox" checked={modules[key] ?? false} onChange={() => toggleModule(key)} className="rounded border-slate-300" />
                      </td>
                      <td className="px-3 py-2.5 font-medium text-slate-900 text-sm">{mod.label}</td>
                      <td className="px-3 py-2.5 text-slate-500 text-xs">{mod.description}</td>
                      <td className="px-3 py-2.5 text-right font-mono text-slate-700 text-sm">{mod.count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
                <p className="text-xs text-blue-700">Inactive staff, students, and out-of-service vehicles are automatically excluded. All copied data can be modified after initialization.</p>
              </div>
            </>
          ) : null
        )}
      ...
      <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-100">
        <p className="text-xs text-slate-500">{totalSelected > 0 ? `≈ ${totalSelected} records will be copied` : 'Select a source year and modules'}</p>
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={onClose}>Cancel</Button>
          <Button size="sm" onClick={handleClone} loading={cloneMutation.isPending} disabled={!sourceYearId || totalSelected === 0}>
        ...
        <div className="mb-4 p-3 bg-slate-50 rounded-lg border border-slate-100">
          ...
          <div className="flex flex-wrap gap-2">
            {teachingStaff.map(t => {
              ...
              const staffId = String(t.id);
              const count = assignmentClasses.reduce((sum, cls) =>
                sum + (cls.sections || []).reduce((sSum, sec) =>
                  sSum + (sec.subject_teachers || []).filter(st => st.staff_id && String(st.staff_id) === staffId).length
                , 0)
              , 0);
...
function HolidayCalendarGrid({ holidays, onDateClick, workingDays = [] }) {
    ...
    <div className="mb-5 bg-white border border-slate-200 rounded-xl p-5 transition-all duration-200 hover:shadow-soft-lg hover:border-slate-300">
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          const dayName = DAY_NAMES_MAP[dayOfWeek];
          const isNonWorking = workingDays.length > 0 ? !workingDays.includes(dayName) : dayOfWeek === 0;

          return (
            <div
              key={day}
              className={`relative min-h-[38px] p-1 rounded-lg text-center transition-all duration-100 ${
                hol ? 'bg-gradient-to-br from-red-50 to-red-100 border border-red-200 cursor-default' :
                isNonWorking ? 'bg-slate-100 cursor-default' :
                isToday ? 'bg-primary-50 border border-primary-200 cursor-pointer hover:ring-2 hover:ring-primary-300' :
                'hover:bg-primary-50 hover:border-primary-200 cursor-pointer border border-transparent'
              }`}
              onMouseEnter={() => hol && setHoveredDay(day)}
              onMouseLeave={() => setHoveredDay(null)}
              onClick={() => {
                if (!hol && !isNonWorking && onDateClick) {
      ...
      {onDateClick && <p className="text-[10px] text-slate-400 mt-2 text-center">Click any date to add a holiday</p>}
    </div>
  );
}
...
function HolidaysTab() {
  ...
  const { data: attConfig } = useAttendanceConfig();
  const workingDays = attConfig?.working_days?.length ? attConfig.working_days : ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        {holidays.length > 0 && <HolidayCalendarGrid holidays={holidays} workingDays={workingDays} onDateClick={(dateStr) => { setForm({ date: dateStr, name: '', type: 'School', description: '' }); setEditingIndex(null); setShowModal(true); }} />}
...
function WorkingDaysSection({ config }) {
  ...
  const savedWorkingDays = config?.working_days?.length ? config.working_days : ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const [workingDays, setWorkingDays] = useState(savedWorkingDays);
  const [saving, setSaving] = useState(false);
  const isDirty = JSON.stringify([...workingDays].sort()) !== JSON.stringify([...savedWorkingDays].sort());

  // Sync state when config changes (after refetch)
  useEffect(() => {
    if (config?.working_days?.length) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put('/admin/settings', { working_days: workingDays });
      qc.invalidateQueries({ queryKey: ['settings', 'attendance-config'] });
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 transition-all duration-200 hover:shadow-soft-lg hover:border-slate-300">
      <div className="flex items-center justify-between mb-4">
        ...
        {isDirty && <Button size="sm" icon={Save} onClick={handleSave} loading={saving}>Save</Button>}
      </div>
...
function GradesTab() {
  ...
  const [editingIndex, setEditingIndex] = useState(null);
  const [deleteIndex, setDeleteIndex] = useState(null);
  const [form, setForm] = useState({ grade: '', min_percentage: '', max_percentage: '', grade_point: '', is_pass: true });
  ...
  const handleSave = () => {
    ...
    // Validate pass/fail ordering: no Pass grade below a Fail grade
    let seenFail = false;
    for (const g of sorted) {
      if (g.is_pass === false) seenFail = true;
      else if (seenFail && g.is_pass !== false) {
        toast.error('Invalid pass/fail order: all grades below a Fail grade must also be Fail. Adjust the pass/fail settings.');
        return;
      }
    }
  ...
  const startAdd = () => {
    setForm({ grade: '', min_percentage: '', max_percentage: '', grade_point: '', is_pass: true });
    setEditingIndex(null);
    setShowModal(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <div className="mb-4 px-3 py-2 rounded-lg bg-blue-50 border border-blue-100 text-xs text-blue-700 space-y-1">
          <p><span className="font-semibold">How ranges work:</span> Min % is inclusive, Max % is exclusive. Example: Grade A (90–100) means 90% ≤ score &lt; 100%. A student scoring exactly 90% gets Grade A. The highest grade should have Max = 100 (which is inclusive for the top grade).</p>
          <p><span className="font-semibold">Pass/Fail:</span> Mark each grade as Pass or Fail. When an exam doesn't have explicit passing marks set, the grade system's pass/fail status is used to determine whether a student passed.</p>
        </div>
        ...
        {isError && !data ? (
          ...
          <GradesTable grades={grades} saveGrades={saveGrades} startEdit={startEdit} setDeleteIndex={setDeleteIndex} toast={toast} />
        ) : (
          <div className="text-center py-12">
            <Award size={24} className="text-slate-300 mx-auto mb-2" />
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setEditingIndex(null); }} title={editingIndex !== null ? 'Edit Grade' : 'Add Grade'} size="sm">
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg border border-slate-200 bg-slate-50">
            <div>
              <p className="text-sm font-medium text-slate-700">Pass Grade</p>
              <p className="text-[11px] text-slate-400">Students with this grade will be marked as Pass</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={form.is_pass} onChange={() => setForm({ ...form, is_pass: !form.is_pass })} className="sr-only peer" />
              <div className="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-emerald-500"></div>
              <span className={`ml-2 text-xs font-semibold ${form.is_pass ? 'text-emerald-600' : 'text-red-600'}`}>{form.is_pass ? 'Pass' : 'Fail'}</span>
            </label>
          </div>
...
function GradesTable({ grades, saveGrades, startEdit, setDeleteIndex, toast }) {
  // Sort grades by percentage (highest first) for display
  const sorted = [...grades].map((g, origIdx) => ({ ...g, _origIdx: origIdx })).sort((a, b) => b.min_percentage - a.min_percentage);

  const handleTogglePassFail = (origIdx) => {
    const clickedGrade = grades[origIdx];
    const sortedByPct = [...grades].sort((a, b) => a.min_percentage - b.min_percentage);
    const clickedSortedIdx = sortedByPct.findIndex(g => g.grade === clickedGrade.grade && g.min_percentage === clickedGrade.min_percentage);

    if (clickedGrade.is_pass !== false) {
      // Setting to Fail → all grades below (lower percentage) also become Fail
      const updated = grades.map(g => {
        if (g.min_percentage <= clickedGrade.min_percentage) return { ...g, is_pass: false };
        return g;
      });
      saveGrades(updated);
    } else {
      // Setting to Pass → all grades above (higher percentage) also become Pass
      const updated = grades.map(g => {
        if (g.min_percentage >= clickedGrade.min_percentage) return { ...g, is_pass: true };
        return g;
      });
      // Validate: no sandwich (Pass below Fail)
      const sortedUpdated = [...updated].sort((a, b) => a.min_percentage - b.min_percentage);
      let seenFail = false;
      for (const g of sortedUpdated) {
        if (g.is_pass === false) seenFail = true;
        else if (seenFail && g.is_pass !== false) {
          toast.error('Invalid: cannot have Pass grades below Fail grades. Fix the grades in between first.');
          return;
        }
      }
      saveGrades(updated);
  ...
  // Find the pass/fail cutoff line index in sorted (highest-first) array
  const cutoffIdx = sorted.findIndex(g => g.is_pass === false);
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100">
            ...
            <th className="text-center py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Pass / Fail</th>
            <th className="text-right py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Actions</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((g, sortedIdx) => {
            const isFail = g.is_pass === false;
            const isAtCutoff = sortedIdx === cutoffIdx;
            return (
              <tr key={g._origIdx} className={`transition-colors ${isFail ? 'bg-red-50/40 hover:bg-red-50/60' : 'hover:bg-slate-50/50'} ${isAtCutoff ? 'border-t-2 border-t-red-300' : 'border-t border-t-slate-50'}`}>
                <td className="py-3 px-4 font-semibold text-slate-900">
                  <span className="flex items-center gap-1.5">
                    {g.grade}
                    {isAtCutoff && <span className="text-[9px] px-1.5 py-0.5 rounded bg-red-100 text-red-600 font-medium">Cutoff</span>}
                  </span>
                </td>
                ...
                <td className="py-3 px-4 text-center">
                  <button
                    onClick={() => handleTogglePassFail(g._origIdx)}
                    ...
                    title={isFail ? 'Click to mark as Pass (grades above will also become Pass)' : 'Click to mark as Fail (grades below will also become Fail)'}
                  >
                    {isFail ? '✗ Fail' : '✓ Pass'}
                  </button>
                </td>
                <td className="py-3 px-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => startEdit(g._origIdx)} className="p-1.5 rounded-lg text-slate-400 hover:text-primary-600 hover:bg-primary-50 transition-colors"><Pencil size={14} /></button>
                    <button onClick={() => setDeleteIndex(g._origIdx)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            );
      ...
      {cutoffIdx >= 0 && (
        <div className="mt-3 px-3 py-2 rounded-lg bg-amber-50 border border-amber-100 text-xs text-amber-700">
          <span className="font-semibold">Pass/Fail cutoff:</span> Grades at or above <span className="font-bold">{sorted[cutoffIdx - 1]?.grade || sorted[0]?.grade}</span> ({sorted[cutoffIdx - 1]?.min_percentage ?? sorted[0]?.min_percentage}%+) are Pass. Grades below are Fail. Click any status to adjust — changes propagate automatically.
        </div>
      )}
    </div>
  );
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        {structures.length > 0 && classes.length > 0 && (() => {
          ...
          const classTotals = classes.map(cls => {
            ...
            const baseTotal = [...generalFees, ...classFees].reduce((sum, fs) => sum + Number(fs.amount) * freqMultiplier(fs.frequency), 0);

            // Check section-specific fees for range using classSections data
            const sections = classSections.filter(cs => cs.class_id === cls.id);
            let minTotal = baseTotal;
            let maxTotal = baseTotal;
            if (sections.length > 0) {
              const sectionFeeExists = structures.some(fs => fs.class_section_id && sections.some(sec => sec.id === fs.class_section_id));
              if (sectionFeeExists) {
                const sectionTotals = sections.map(sec => {
                  const sectionFees = structures.filter(fs => fs.class_section_id === sec.id);
                  return baseTotal + sectionFees.reduce((sum, fs) => sum + Number(fs.amount) * freqMultiplier(fs.frequency), 0);
                });
                minTotal = Math.min(...sectionTotals);
                maxTotal = Math.max(...sectionTotals);
              }
            }

            return { cls, annualTotal: baseTotal, minTotal, maxTotal, hasRange: minTotal !== maxTotal };
          });
          const allTotals = classTotals.flatMap(ct => [ct.minTotal, ct.maxTotal]).filter(t => t > 0);
          ...
          <div className="mb-4 grid grid-cols-2 md:grid-cols-5 gap-2">
            {classTotals.map(({ cls, annualTotal, minTotal, maxTotal, hasRange }) => {
              const displayTotal = hasRange ? maxTotal : annualTotal;
              const isHighest = displayTotal > 0 && displayTotal === highestTotal && highestTotal !== lowestTotal;
              const isLowest = minTotal > 0 && minTotal === lowestTotal && highestTotal !== lowestTotal;
              return (
                <div key={cls.id} className={`px-3 py-2 rounded-lg border text-center ${isHighest ? 'bg-red-50 border-red-200' : isLowest ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  {hasRange ? (
                    <p className="text-sm font-bold text-slate-800 mt-0.5">₹{minTotal.toLocaleString()} - ₹{maxTotal.toLocaleString()}</p>
                  ) : (
