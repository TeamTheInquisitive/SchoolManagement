...
import { Plus, Pencil, Trash2, Users, CheckCircle, GraduationCap, BookOpen, ChevronDown, ChevronUp, BarChart3 } from 'lucide-react';
...
export default function TeacherPrivilegesPage() {
  ...
  const workloadData = teachers.map(t => ({
    name: (t.user?.full_name || '').split(' ')[0] || 'Unknown',
    fullName: t.user?.full_name || 'Unknown',
    assignments: t.class_assignments?.length || 0,
  })).sort((a, b) => b.assignments - a.assignments);

  const getBarColor = (assignments) => {
    if (assignments > 15) return '#ef4444'; // red - overloaded
    if (assignments > 10) return '#f59e0b'; // amber - moderate
    return '#22c55e'; // green - normal
  };

  const overloadedCount = workloadData.filter(t => t.assignments > 15).length;
  const unassignedCount = workloadData.filter(t => t.assignments === 0).length;
  ...
  return (
    <div>
      ...
      {/* Workload Overview */}
      {teachers.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl mb-6 transition-all duration-200 hover:shadow-soft-lg hover:border-slate-300">
          <button
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <h3 className="text-base font-semibold text-slate-900">Workload Overview</h3>
                <p className="text-xs text-slate-500 mt-0.5">
                  {overloadedCount > 0 && <span className="text-red-600 font-medium">{overloadedCount} teacher{overloadedCount !== 1 ? 's' : ''} overloaded</span>}
                  {overloadedCount > 0 && unassignedCount > 0 && <span className="mx-1.5 text-slate-300">|</span>}
                  {unassignedCount > 0 && <span className="text-amber-600 font-medium">{unassignedCount} teacher{unassignedCount !== 1 ? 's' : ''} unassigned</span>}
                  {overloadedCount === 0 && unassignedCount === 0 && <span className="text-emerald-600 font-medium">All teachers have balanced workloads</span>}
                </p>
              </div>
            </div>
            {workloadExpanded ? <ChevronUp size={20} className="text-slate-400" /> : <ChevronDown size={20} className="text-slate-400" />}
          </button>
            ...
            <div className="px-4 md:px-5 pb-4 md:pb-5">
              <div className="flex items-center gap-4 mb-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-500">Normal (1-10)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-amber-500" />
                  <span className="text-xs text-slate-500">Moderate (11-15)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-sm bg-red-500" />
                  <span className="text-xs text-slate-500">Overloaded (&gt;15)</span>
                </div>
              </div>
              <ResponsiveContainer width="100%" height={Math.max(200, workloadData.length * 36)}>
                <BarChart data={workloadData} layout="vertical" margin={{ left: 10, right: 30, top: 5, bottom: 5 }}>
                  <XAxis type="number" tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: '#64748b' }} axisLine={false} tickLine={false} width={80} />
                  <Tooltip
                    contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px -4px rgba(0,0,0,0.1)', padding: '8px 12px' }}
                    formatter={(value, name, props) => [`${value} assignment${value !== 1 ? 's' : ''}`, props.payload.fullName]}
                    labelFormatter={() => ''}
                  />
                  <Bar dataKey="assignments" radius={[0, 6, 6, 0]} maxBarSize={24}>
                    {workloadData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={getBarColor(entry.assignments)} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
