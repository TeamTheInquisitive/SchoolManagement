...
import { Button, Badge, Modal, ConfirmDialog, SearchableSelect, useToast, exportStyledExcel, generateFeeReceipt, useSortableData, Breadcrumb, HoverCard, usePagination, DataTable, DatePicker , AnimatedNumber } from 'school-erp-ui-shared';
...
export default function FeeManagementPage() {
  ...
  const dateFilteredFees = fees.filter(f => {
    if (fromDate && f.due_date && f.due_date < fromDate) return false;
    if (toDate && f.due_date && f.due_date > toDate) return false;
    return true;
  });
  const { sortedData, sortConfig, requestSort } = useSortableData(dateFilteredFees);
  ...
  // Class-wise fee collection progress data
  const classWiseData = useMemo(() => {
    if (!fees.length) return [];
    const classMap = {};
    fees.forEach(f => {
      const cls = f.class_name || 'Unknown';
      if (!classMap[cls]) classMap[cls] = { className: cls, totalAmount: 0, paidAmount: 0, count: 0 };
      classMap[cls].totalAmount += Number(f.total_amount || 0);
      classMap[cls].paidAmount += Number(f.total_paid || f.paid || 0);
  ...
  const statusPieData = useMemo(() => {
    if (!fees.length) return [];
    const statusMap = { Paid: 0, Pending: 0, Overdue: 0, Partial: 0 };
    fees.forEach(f => {
      const status = (f.status || 'pending').charAt(0).toUpperCase() + (f.status || 'pending').slice(1).toLowerCase();
      if (statusMap[status] !== undefined) statusMap[status] += 1;
      else statusMap['Pending'] += 1;
    });
    return Object.entries(statusMap).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value }));
  }, [fees]);

  // Auto-fill payment amount with pending balance when modal opens
  useEffect(() => {
    if (paymentDialog) {
      const pendingBalance = Number(paymentDialog.pending_amount || paymentDialog.total_pending || 0) || (Number(paymentDialog.total_amount || 0) - Number(paymentDialog.paid_amount || paymentDialog.total_paid || 0));
      if (pendingBalance > 0) {
  ...
  const handleExportExcel = async () => {
    if (exporting) return;
    setExporting(true);
    toast.info('Exporting fee records... Please wait.');
    try {
      const { fetchAllPages } = await import('../../utils/exportAllPages');
      const allFees = await fetchAllPages(ENDPOINTS.fees.list, {
        ...(classFilter && { class_name: classFilter }),
        ...(sectionFilter && { section: sectionFilter }),
        ...(statusFilter && { status: statusFilter }),
      }, 100);
      const fp = [classFilter && `Class_${classFilter}`, sectionFilter && `Section_${sectionFilter}`, statusFilter && statusFilter].filter(Boolean);
      const fn = fp.length > 0 ? `${fp.join('_')}_Fee_Records` : 'All_Fee_Records';
      const fd = fp.length > 0 ? `Filters: ${[classFilter && `Class: ${classFilter}`, sectionFilter && `Section: ${sectionFilter}`, statusFilter && `Status: ${statusFilter}`].filter(Boolean).join(' | ')}` : '';
      const headers = ['Student', 'Class', 'Section', 'Components', 'Total', 'Paid', 'Pending', 'Status'];
      const rows = allFees.map(f => [f.student_name, f.class_name || '', f.section || '', f.components_count || 0, f.total_amount || 0, f.total_paid || 0, f.total_pending || 0, f.status || '']);
      await exportStyledExcel({ filename: fn, schoolInfo: getSchoolInfo(schoolProfile), reportTitle: `Fee Records${fd ? ` (${fd})` : ''}`, headers, rows });
      toast.success(`Exported ${allFees.length} fee records`);
    } catch {
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button variant="secondary" icon={Send} onClick={() => setConfirmReminder(true)}>{selectedIds.length > 0 ? `Send Reminder (${selectedIds.length})` : 'Send Reminders (All Overdue)'}</Button>
      ...
      {fees.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl p-5 mb-4">
          <h3 className="text-sm font-semibold text-slate-800 mb-4">Class-wise Fee Collection Progress</h3>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            {/* Progress bars */}
            <div className="lg:col-span-2 space-y-3 max-h-[280px] overflow-y-auto pr-2">
              {classWiseData.map(cls => {
                const pct = cls.totalAmount > 0 ? ((cls.paidAmount / cls.totalAmount) * 100).toFixed(1) : 0;
                return (
                  <div key={cls.className} className="group">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-sm font-medium text-slate-700">Class {cls.className}</span>
                      <span className="text-xs text-slate-500">
                        {'₹'}{Math.round(cls.paidAmount).toLocaleString()} / {'₹'}{Math.round(cls.totalAmount).toLocaleString()}
                        <span className={`ml-2 font-semibold ${Number(pct) >= 75 ? 'text-emerald-600' : Number(pct) >= 50 ? 'text-blue-600' : 'text-amber-600'}`}>({pct}%)</span>
                      </span>
                    </div>
                    <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${Number(pct) >= 75 ? 'bg-emerald-500' : Number(pct) >= 50 ? 'bg-blue-500' : 'bg-amber-500'}`}
                        style={{ width: `${Math.min(Number(pct), 100)}%` }}
                      />
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5">{cls.count} student{cls.count !== 1 ? 's' : ''}</p>
                  </div>
                );
              })}
              {classWiseData.length === 0 && <p className="text-sm text-slate-400 text-center py-4">No class data available</p>}
            </div>
            {/* Status pie chart */}
            <div className="flex flex-col items-center justify-center">
              <p className="text-xs font-medium text-slate-500 mb-2">Status Breakdown</p>
              {statusPieData.length > 0 ? (
                <>
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                      <Pie data={statusPieData} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" nameKey="name" paddingAngle={2}>
                        {statusPieData.map((entry, index) => {
                          const colors = { Paid: '#22c55e', Pending: '#f59e0b', Overdue: '#ef4444', Partial: '#3b82f6' };
                    ...
                    {statusPieData.map(entry => {
                      ...
                      return (
                        <span key={entry.name} className="flex items-center gap-1.5 text-xs text-slate-600">
                          <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: colors[entry.name] || '#94a3b8' }} />
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <span className="text-sm font-medium text-slate-600">Due Date Range:</span>
      ...
      <DataTable
        ...
        headerExtra={<Button variant="secondary" size="sm" icon={Download} onClick={handleExportExcel} disabled={exporting} loading={exporting}>{exporting ? 'Exporting...' : 'Export Excel'}</Button>}
      ...
      <ConfirmDialog
        ...
        message={selectedIds.length > 0
          ? `Are you sure you want to send fee payment reminders to ${selectedIds.length} selected student(s)?`
          : 'Are you sure you want to send fee payment reminders to all students with overdue payments?'}
        onConfirm={() => {
          sendReminderMutation.mutate(
            selectedIds.length > 0
              ? { target_group: 'Selected', student_ids: selectedIds, message: 'This is a reminder that your fee payment is overdue. Please clear the dues at the earliest.', send_via: 'in_app' }
              : { target_group: 'Overdue', message: 'This is a reminder that your fee payment is overdue. Please clear the dues at the earliest.', send_via: 'in_app' },
            {
              onSuccess: () => { setSelectedIds([]); setConfirmReminder(false); toast.success('Reminders sent successfully'); },
              onError: (err) => { setConfirmReminder(false); toast.error(err.response?.data?.error || err.response?.data?.detail || 'Failed to send reminders'); },
            }
          );
        }}
...
function ReceiptModal({ fee, payment }) {
  ...
  const handlePrint = () => {
    generateFeeReceipt({
      ...
      schoolName,
      schoolInfo: getSchoolInfo({ school_name: schoolName }),
    });
  };
...
function PaymentModalContent({ paymentDialog, paymentAmount, setPaymentAmount, paymentMethod, setPaymentMethod, paymentRemarks, setPaymentRemarks }) {
  const maxPayable = Number(paymentDialog.pending_amount || paymentDialog.total_pending || 0) || (Number(paymentDialog.total_amount || 0) - Number(paymentDialog.paid_amount || paymentDialog.total_paid || 0));
  const totalAmount = Number(paymentDialog.total_amount || 0);
  const paidSoFar = Number(paymentDialog.paid_amount || paymentDialog.total_paid || 0);

  // Fetch recent payment history for context
  const { data: historyData } = useQuery({
    queryKey: ['payment-modal-history', paymentDialog.student_id],
    queryFn: () => api.get(`/admin/students/${paymentDialog.student_id}/fee-history`).then(r => r.data).catch(() => null),
    enabled: !!paymentDialog.student_id,
    staleTime: 30000,
  });

  const recentPayments = (historyData?.payments || []).slice(-3);

  return (
    <div className="space-y-4">
      {/* Prominent Pending Balance Banner */}
      <div className="bg-gradient-to-r from-red-50 to-amber-50 border border-red-200 rounded-xl p-4 text-center">
        <p className="text-xs font-medium text-red-500 uppercase tracking-wider">Outstanding Balance</p>
        <p className="text-2xl font-bold text-red-700 mt-1">₹{maxPayable.toLocaleString()}</p>
        <div className="flex items-center justify-center gap-4 mt-2">
          <span className="text-xs text-slate-500">Total: ₹{totalAmount.toLocaleString()}</span>
          <span className="text-xs text-emerald-600">Paid: ₹{paidSoFar.toLocaleString()}</span>
        </div>
      </div>
      ...
      {recentPayments.length > 0 && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
          <p className="text-xs font-semibold text-blue-700 mb-2 flex items-center gap-1.5">
            <History size={12} /> Previous Payments
          </p>
            ...
            {recentPayments.map((p, i) => (
              <div key={p.id || i} className="flex items-center justify-between text-xs">
                <span className="text-slate-600">
                  ...
                  <span className="text-slate-400 ml-1.5">via {p.method || p.payment_method || 'Cash'}</span>
                </span>
                <span className="text-slate-400">{p.date || p.payment_date || p.paid_on || ''}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mt-1">
          <p className="text-xs text-slate-400">Maximum: ₹{maxPayable.toLocaleString()}</p>
          {Number(paymentAmount) > 0 && Number(paymentAmount) < maxPayable && (
            <button onClick={() => setPaymentAmount(String(maxPayable))} className="text-xs text-primary-600 hover:text-primary-700 font-medium transition-colors">Pay Full Balance</button>
          )}
        </div>
      </div>
